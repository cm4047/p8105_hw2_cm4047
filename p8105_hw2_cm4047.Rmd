...
The dataset is not tidy enough. For example, the values for line variable contain "4 Avenue", "6 Avenue", and "42nd St Shuttle". These three values are not in the same format. And, there are so many routes(from "route1" to "route2"), which is so complex.   

There are `r nrow(distinct(nyc_transit_df, line, station_name))` distinct stations.  
There are `r nrow(filter(nyc_transit_df, ada == "TRUE"))` stations with ADA compliant.  
`r  nrow(filter(nyc_transit_df, vending == "NO", entry == "TRUE"))/nrow(filter(nyc_transit_df, entry == "TRUE"))` of station entrances/exits without vending allow entrance.
...
There are `r filter(nyc_transit_tidy, route_name == "A") %>% distinct(line, station_name) %>%  nrow()` distinct stations serve the A train. Of the `r filter(nyc_transit_tidy, route_name == "A") %>% distinct(line, station_name) %>%  nrow()` stations that serve the A train, `r filter(nyc_transit_tidy, route_name == "A", ada == "TRUE") %>%  distinct(line, station_name) %>%  nrow()` are ADA compliant.
